...
func (b *backend) sign(
	...
	messageHashStr := data.Get("hash").(string)
	if len(message) > 0 {
		if len(messageHashStr) > 0 {
			return nil, errors.New("cannot specify both `message` and `hash`")
		}
		messageHash = accounts.TextHash([]byte(message))
	} else {
		messageHash = []byte(common.FromHex(messageHashStr))
	}

	account, err := b.retrieveAccount(ctx, req, from)
	if err != nil {
		b.Logger().Error(
			"Failed to retrieve the signing account",
			"address", from,
			"error", err,
		)
	...
	if err != nil {
		b.Logger().Error(
			"Error reconstructing private key from retrieved hex",
			"error", err,
		)
		err = fmt.Errorf("error reconstructing private key from retrieved hex")
		return nil, err
	}
	defer ZeroKey(privateKey)
